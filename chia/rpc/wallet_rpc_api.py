...
from chia.rpc.util import potentially_inside_lock, tx_endpoint
...
class WalletRpcApi:
    ...
    @tx_endpoint
    async def create_new_wallet(self, request: Dict, tx_config: TXConfig) -> EndpointResult:
    ...
    @tx_endpoint
    async def send_transaction(self, request, tx_config: TXConfig) -> EndpointResult:
        if await self.service.wallet_state_manager.synced() is False:
    ...
    @tx_endpoint
    async def spend_clawback_coins(self, request, tx_config: TXConfig) -> EndpointResult:
    ...
    @tx_endpoint
    async def select_coins(self, request, tx_config: TXConfig) -> EndpointResult:
        if await self.service.wallet_state_manager.synced() is False:
    ...
    @tx_endpoint
    async def send_notification(self, request, tx_config: TXConfig) -> EndpointResult:
    ...
    @tx_endpoint
    async def sign_message_by_id(self, request, tx_config: TXConfig) -> EndpointResult:
        """
    ...
    @tx_endpoint
    async def cat_spend(self, request, tx_config: TXConfig, hold_lock: bool) -> EndpointResult:
        if await self.service.wallet_state_manager.synced() is False:
            raise ValueError("Wallet needs to be fully synced.")
        wallet_id = uint32(request["wallet_id"])
        wallet = self.service.wallet_state_manager.get_wallet(id=wallet_id, required_type=CATWallet)
    ...
    @tx_endpoint
    async def create_offer_for_ids(self, request, tx_config: TXConfig) -> EndpointResult:
    ...
    @tx_endpoint
    async def take_offer(self, request, tx_config: TXConfig) -> EndpointResult:
        offer_hex: str = request["offer"]
        offer = Offer.from_bech32(offer_hex)
        fee: uint64 = uint64(request.get("fee", 0))
    ...
    @tx_endpoint
    async def cancel_offer(self, request: Dict, tx_config: TXConfig) -> EndpointResult:
    ...
    @tx_endpoint
    async def cancel_offers(self, request, tx_config: TXConfig) -> EndpointResult:
        secure = request["secure"]
    ...
    @tx_endpoint
    async def did_update_recovery_ids(self, request, tx_config: TXConfig) -> EndpointResult:
        wallet_id = uint32(request["wallet_id"])
        wallet = self.service.wallet_state_manager.get_wallet(id=wallet_id, required_type=DIDWallet)
        recovery_list = []
        success: bool = False
    ...
    @tx_endpoint
    async def did_message_spend(self, request, tx_config: TXConfig) -> EndpointResult:
        wallet_id = uint32(request["wallet_id"])
        wallet = self.service.wallet_state_manager.get_wallet(id=wallet_id, required_type=DIDWallet)
    ...
    @tx_endpoint
    async def did_find_lost_did(self, request, tx_config: TXConfig) -> EndpointResult:
        """
    ...
    @tx_endpoint
    async def did_update_metadata(self, request, tx_config: TXConfig) -> EndpointResult:
        wallet_id = uint32(request["wallet_id"])
        wallet = self.service.wallet_state_manager.get_wallet(id=wallet_id, required_type=DIDWallet)
        metadata: Dict[str, str] = {}
    ...
    @tx_endpoint
    async def did_get_did(self, request, tx_config: TXConfig) -> EndpointResult:
        wallet_id = uint32(request["wallet_id"])
        wallet = self.service.wallet_state_manager.get_wallet(id=wallet_id, required_type=DIDWallet)
    ...
    @tx_endpoint
    async def did_create_attest(self, request, tx_config: TXConfig) -> EndpointResult:
        wallet_id = uint32(request["wallet_id"])
        wallet = self.service.wallet_state_manager.get_wallet(id=wallet_id, required_type=DIDWallet)
        async with self.service.wallet_state_manager.lock:
    ...
    @tx_endpoint
    async def did_get_current_coin_info(self, request, tx_config: TXConfig) -> EndpointResult:
        wallet_id = uint32(request["wallet_id"])
        did_wallet = self.service.wallet_state_manager.get_wallet(id=wallet_id, required_type=DIDWallet)
        my_did = encode_puzzle_hash(
            bytes32.from_hexstr(did_wallet.get_my_DID()), AddressType.DID.hrp(self.service.config)
    ...
    @tx_endpoint
    async def did_transfer_did(self, request, tx_config: TXConfig) -> EndpointResult:
        if await self.service.wallet_state_manager.synced() is False:
            raise ValueError("Wallet needs to be fully synced.")
        wallet_id = uint32(request["wallet_id"])
        did_wallet = self.service.wallet_state_manager.get_wallet(id=wallet_id, required_type=DIDWallet)
    ...
    ##########################################################################################
    @tx_endpoint
    async def nft_mint_nft(self, request, tx_config: TXConfig) -> EndpointResult:
    ...
    @tx_endpoint
    async def nft_set_nft_did(self, request, tx_config: TXConfig):
        wallet_id = uint32(request["wallet_id"])
        nft_wallet = self.service.wallet_state_manager.get_wallet(id=wallet_id, required_type=NFTWallet)
    ...
    @tx_endpoint
    async def nft_set_did_bulk(self, request, tx_config: TXConfig):
        """
    ...
    @tx_endpoint
    async def nft_transfer_bulk(self, request, tx_config: TXConfig):
        """
    ...
    @tx_endpoint
    async def nft_transfer_nft(self, request, tx_config: TXConfig) -> EndpointResult:
        wallet_id = uint32(request["wallet_id"])
        address = request["target_address"]
        if isinstance(address, str):
            puzzle_hash = decode_puzzle_hash(address)
    ...
    @tx_endpoint
    async def nft_add_uri(self, request, tx_config: TXConfig) -> EndpointResult:
        wallet_id = uint32(request["wallet_id"])
    ...
    @tx_endpoint
    async def nft_mint_bulk(self, request, tx_config: TXConfig) -> EndpointResult:
        if await self.service.wallet_state_manager.synced() is False:
            raise ValueError("Wallet needs to be fully synced.")
        wallet_id = uint32(request["wallet_id"])
        nft_wallet = self.service.wallet_state_manager.get_wallet(id=wallet_id, required_type=NFTWallet)
    ...
    @potentially_inside_lock
    @tx_endpoint
    async def create_signed_transaction(self, request, tx_config: TXConfig, hold_lock: bool) -> EndpointResult:
    ...
    ##########################################################################################
    @tx_endpoint
    async def pw_join_pool(self, request, tx_config: TXConfig) -> EndpointResult:
        fee = uint64(request.get("fee", 0))
        wallet_id = uint32(request["wallet_id"])
    ...
    @tx_endpoint
    async def pw_self_pool(self, request, tx_config: TXConfig) -> EndpointResult:
    ...
    @tx_endpoint
    async def pw_absorb_rewards(self, request, tx_config: TXConfig) -> EndpointResult:
    ...
    ##########################################################################################
    @tx_endpoint
    async def create_new_dl(self, request, tx_config: TXConfig) -> Dict:
    ...
    @tx_endpoint
    async def dl_update_root(self, request, tx_config: TXConfig) -> Dict:
        """Get the singleton record for the latest singleton of a launcher ID"""
        if self.service.wallet_state_manager is None:
            raise ValueError("The wallet service is not currently initialized")
    ...
    @tx_endpoint
    async def dl_update_multiple(self, request, tx_config: TXConfig) -> Dict:
    ...
    @tx_endpoint
    async def dl_new_mirror(self, request, tx_config: TXConfig) -> Dict:
    ...
    @tx_endpoint
    async def dl_delete_mirror(self, request, tx_config: TXConfig) -> Dict:
    ...
    ##########################################################################################
    @tx_endpoint
    async def vc_mint(self, request, tx_config: TXConfig) -> Dict:
        """
    ...
    @tx_endpoint
    async def vc_spend(self, request, tx_config: TXConfig) -> Dict:
        """
    ...
    @tx_endpoint
    async def vc_revoke(self, request, tx_config: TXConfig) -> Dict:
        """
